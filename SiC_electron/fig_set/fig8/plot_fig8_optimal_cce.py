#!/usr/bin/env python3
"""Plot Fig. 8: maximum CCE versus trap density."""
...
OUT_BASE = THIS_DIR / "fig8_optimal_CCE_vs_Nt"
OUT_CSV = THIS_DIR / "fig8_optimal_CCE_vs_Nt.csv"
...
def main() -> None:
    ...
    for source, marker in zip(SOURCE_ORDER, markers):
        ...
        y_values = [float(next(row for row in selected if row["nt"] == nt)["cce_percent"]) for nt in NT_ORDER]
        ax.plot(
            x_pos,
            y_values,
            marker=marker,
