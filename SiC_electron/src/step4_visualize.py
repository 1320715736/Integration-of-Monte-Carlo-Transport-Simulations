...
def write_png(output_path: Path, rgb: np.ndarray) -> None:
...
def normalize_nonzero_log(values: np.ndarray) -> np.ndarray:
    ...
    log_values = np.log10(values[nonzero])
    vmin = float(log_values.min())
    vmax = float(log_values.max())
    if math.isclose(vmin, vmax):
        normalized[nonzero] = 1.0
        return normalized

    normalized[nonzero] = (log_values - vmin) / (vmax - vmin)
...
def apply_colormap(normalized: np.ndarray) -> np.ndarray:
    ...
    stops = COLORMAP_STOPS[:, 0]
    for channel in range(3):
        rgb[..., channel] = np.interp(
            normalized,
            stops,
            COLORMAP_STOPS[:, channel + 1],
        ).astype(np.uint8)
    return rgb


def downsample_image_plane(image_plane: np.ndarray, max_width: int, max_height: int) -> np.ndarray:
    height, width = image_plane.shape
    step_y = max(1, math.ceil(height / max_height))
    step_x = max(1, math.ceil(width / max_width))
    return image_plane[::step_y, ::step_x]


def add_frame(rgb: np.ndarray, padding: int = 12) -> np.ndarray:
    height, width, _ = rgb.shape
    canvas = np.full((height + 2 * padding, width + 2 * padding, 3), 255, dtype=np.uint8)
    canvas[padding : padding + height, padding : padding + width] = rgb
    canvas[padding - 1 : padding + height + 1, padding - 1] = 20
    canvas[padding - 1 : padding + height + 1, padding + width] = 20
    canvas[padding - 1, padding - 1 : padding + width + 1] = 20
    canvas[padding + height, padding - 1 : padding + width + 1] = 20
    return canvas


def save_heatmap_fallback_png(output_path: Path, grid: np.ndarray) -> None:
    display = np.asarray(grid.T[::-1, :], dtype=float)
    display = downsample_image_plane(display, max_width=1400, max_height=900)
    normalized = normalize_nonzero_log(display)
    rgb = apply_colormap(normalized)
    rgb[display <= 0.0] = np.array([245, 245, 245], dtype=np.uint8)
    write_png(output_path, add_frame(rgb))


def save_mesh_preview_fallback_png(output_path: Path, grid: np.ndarray) -> None:
    ...
    if g_plot.shape[0] < 2 or g_plot.shape[1] < 2:
        g_plot = grid[max(0, x0) : min(grid.shape[0], x0 + 20), max(0, z0) : min(grid.shape[1], z0 + 20)]

    display = np.asarray(g_plot.T[::-1, :], dtype=float)
    normalized = normalize_nonzero_log(display)
    rgb = apply_colormap(normalized)
    rgb[display <= 0.0] = np.array([245, 245, 245], dtype=np.uint8)

    cell_size = max(8, min(20, 720 // max(display.shape[0], 1), 1200 // max(display.shape[1], 1)))
    upscaled = np.repeat(np.repeat(rgb, cell_size, axis=0), cell_size, axis=1)
    line_color = np.array([50, 50, 50], dtype=np.uint8)
    upscaled[::cell_size, :, :] = line_color
    upscaled[:, ::cell_size, :] = line_color
    upscaled[-1, :, :] = line_color
    upscaled[:, -1, :] = line_color
    write_png(output_path, add_frame(upscaled))


def save_histogram_fallback_png(output_path: Path, grid: np.ndarray) -> None:
    ...
    image[top : top + plot_h + 1, left] = 30
    image[top + plot_h, left : left + plot_w + 1] = 30
    ...
    for index, count in enumerate(counts):
        x0 = left + int(round((np.log10(edges[index]) - log_min) / (log_max - log_min) * plot_w))
        x1 = left + int(round((np.log10(edges[index + 1]) - log_min) / (log_max - log_min) * plot_w))
        bar_height = 0 if max_count == 0 else int(round(count / max_count * plot_h))
        y0 = top + plot_h - bar_height
        x1 = max(x1, x0 + 1)
        image[y0 : top + plot_h, x0:x1] = np.array([31, 119, 180], dtype=np.uint8)

    for fraction in (0.25, 0.5, 0.75, 1.0):
        y = top + plot_h - int(round(plot_h * fraction))
        image[y, left : left + plot_w + 1] = np.array([220, 220, 220], dtype=np.uint8)
...
def main() -> int:
    ...
    if plt is not None:
        ...
        save_heatmap_fallback_png(output_dir / "intermediate_picture_heatmap.png", grid)
        save_mesh_preview_fallback_png(output_dir / "intermediate_picture_mesh_preview.png", grid)
        save_histogram_fallback_png(output_dir / "intermediate_picture_histogram.png", grid)
        print(f"matplotlib is not installed; saved dependency-free PNG figures to {output_dir}")
