...
$^{14}\mathrm{C}$ 探测的核心并不是高 $Z$ 材料对强穿透辐射的阻止能力，而是对低能连续 β 电子在近表面和浅层耗尽区内产生的弱电荷信号进行稳定收集。对于这一类源项，探测材料需要同时满足低暗电流、低噪声、高电场下的厚耗尽能力以及长期辐照稳定性。Si 虽然工艺成熟，但较窄禁带会增加室温漏电流和温度敏感性；CdTe/CZT 等高 $Z$ 材料更适合 X/γ 探测，其材料缺陷和极化效应并不直接对应 $^{14}\mathrm{C}$ 低能电子的主要设计矛盾；金刚石具有优良的低噪声和辐照稳定性，但大面积、厚有源层和器件集成成本较高。相比之下，4H-SiC 能在较低暗电流下形成较强漂移电场，并已有 β 源和 SiC 探测器研究基础 [2,3,10,15-20]，因此更适合本文关注的 $^{14}\mathrm{C}$ 连续谱沉积、i 区厚度和外延缺陷共同约束问题。
...
需要说明的是，$5.6\times10^{12}\ \mathrm{cm^{-3}}$ 的 i 区净掺杂属于较理想的低掺杂设计情形。该取值使 $W_i=120\ \mu\mathrm{m}$ 器件的解析全耗尽电压约为 $75\ \mathrm{V}$，从而可以在相对温和的反偏条件下比较不同厚度器件的收集趋势。已有 4H-SiC 探测器外延层的有效掺杂常见于 $10^{13}$-$10^{14}\ \mathrm{cm^{-3}}$ 量级 [11,25,28]；若采用更高净掺杂，厚 i 区的全耗尽电压会按 $N_DW_i^2$ 增大，并可能引入更强的偏压和漏电约束。因此，本文中的掺杂设定应理解为面向厚耗尽 $^{14}\mathrm{C}$ 探测的低净掺杂基准，而不是对常规商用外延层掺杂水平的默认假设。
...
为确定从 Geant4 转换到 TCAD 可能引入的额外误差，本文采用上述能量-电荷归一化基准，比较 Geant4 沉积能量对应的电子-空穴对数和 TCAD 生成率积分。映射生成率积分为
