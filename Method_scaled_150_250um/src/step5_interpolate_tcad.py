...
def interpolate_regular_grid(
    ...
    x = np.asarray(x_centers_um, dtype=float)
    z = np.asarray(z_centers_um, dtype=float)
    values = np.asarray(optical_generation_grid, dtype=float)
    ...
    x_idx = np.searchsorted(x, qx[inside], side="right") - 1
    z_idx = np.searchsorted(z, qz[inside], side="right") - 1
    x_idx = np.clip(x_idx, 0, x.size - 2)
    ...
    tx = np.divide(
        qx[inside] - x0,
        x1 - x0,
        out=np.zeros_like(qx[inside]),
        where=(x1 - x0) != 0.0,
    )
    tz = np.divide(
        qz[inside] - z0,
        z1 - z0,
        out=np.zeros_like(qz[inside]),
        where=(z1 - z0) != 0.0,
    )

    f00 = values[x_idx, z_idx]
    f10 = values[x_idx + 1, z_idx]
    f01 = values[x_idx, z_idx + 1]
    f11 = values[x_idx + 1, z_idx + 1]
...
def interpolate_to_tcad_vertices(
    ...
    interpolated = interpolate_regular_grid(
        x_centers_um,
        z_centers_um,
